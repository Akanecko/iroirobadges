...
const config = {
	...
	"changelog": [{
		...
		"title": "修正",
		"items": [
			"ブーストバッジ設定とバッジ設定をリセットしたときに表示される言語を日本語に変更しました。"
		]
	}]
};
						...
						tree.splice(7, 0, React.createElement(contextmenu_namespaceObject.MenuGroup, null, React.createElement(contextmenu_namespaceObject.MenuItem, {
							...
							children: [modules_UserFlags.map((flag => {
								...
								action: () => {
									...
									this.saveSettings(settings);
									external_PluginApi_namespaceObject.Toasts.success(`<strong>${props.user}</strong> のブーストバッジ設定をリセットしました`);
								}
							}))]), React.createElement(contextmenu_namespaceObject.MenuGroup, null, React.createElement(contextmenu_namespaceObject.MenuItem, {
								color: "colorDanger",
								label: "設定をリセット",
								...
								action: () => {
									...
									external_PluginApi_namespaceObject.Toasts.success(`<strong>${props.user}</strong> のバッジ設定をリセットしました`);
								}
							}))]
						})));
					}));
